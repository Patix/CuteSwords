...
namespace Autotiles3D
{
    ...
    public class Autotiles3D_TileLayer : MonoBehaviour, ISerializationCallbackReceiver
    {
        ...
        public Vector3 InternalToLocalPoint(Vector3 internalPosition)
        {
            float unit = Grid != null ? Grid.Unit : 1f;
            return _positionOffset + internalPosition * unit;
        }

        public Vector3 InternalToWorldPoint(Vector3 internalPosition)
        {
            return Grid != null ? Grid.ToWorldPoint(InternalToLocalPoint(internalPosition)) : transform.TransformPoint(InternalToLocalPoint(internalPosition));
        }

        public Vector3 InternalToWorldPoint(Vector3Int internalPosition)
        {
            return InternalToWorldPoint((Vector3)internalPosition);
        }

        public Vector3Int WorldToInternalPosition(Vector3 worldPosition)
        {
            Vector3 localPoint = Grid != null ? Grid.transform.InverseTransformPoint(worldPosition) : transform.InverseTransformPoint(worldPosition);
            localPoint -= _positionOffset;

            float unit = Grid != null && !Mathf.Approximately(Grid.Unit, 0f) ? Grid.Unit : 1f;
            return Vector3Int.RoundToInt(localPoint / unit);
        }
